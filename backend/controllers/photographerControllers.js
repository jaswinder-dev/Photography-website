...
export const updateAvatar = catchAsyncError(async (req, res, next) => {
    ...
    await Photographer.findByIdAndUpdate(id, { avatar: `http://${process.env.HOST}:${process.env.PORT}/${req.file.filename}` });
    res.status(200).json({
        success: true,
...
export const DeleteAvatar = catchAsyncError(async (req, res, next) => {
    ...
    await Photographer.findByIdAndUpdate(id, { avatar: `http://${process.env.HOST}:${process.env.PORT}/default.png` });
    res.status(200).json({
        success: true,
